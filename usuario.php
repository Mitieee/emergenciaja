...
if (isset($_GET['obter_localizacao_emergencia'])) {
    header('Content-Type: application/json');
    $id_emergencia = $_GET['id_emergencia']; 
...
<head>
    ...
    <title>Usuário - Localização Compartilhada</title>
...
<body>
    ...
    <img src="imagem/emergenciaja.png" alt="emergenciaja" style="width: 150px; height: 80px; ">
        <h1>Usuário - Localização Compartilhada <?= htmlspecialchars(ucfirst($emergencia['tipo_emergencia'])) ?></h1>
    ...
    <script src="https://maps.googleapis.com/maps/api/js?key=<?php echo getenv('GOOGLE_API_KEY'); ?>&callback=initMap" async defer></script>
    <script>
        let mapa, marcaUsuario, marcaEmergencia;
        function initMap() {
            const coordenadas = {
            ...
            new google.maps.InfoWindow({ content: "Você está aqui" }).open(mapa, marcaUsuario);
            if (navigator.geolocation) {
                navigator.geolocation.watchPosition(
                    (posicao) => {
                        ...
                        marcaUsuario.setPosition(novaPosicao);
            ...
            setInterval(atualizarLocalizacaoEmergencia, 3000);
        }
        ...
        function atualizarLocalizacaoEmergencia() {
            fetch(`usuario.php?obter_localizacao_emergencia=1&id_emergencia=<?= $id_emergencia ?>`)
                    ...
                    if (localizacao && localizacao.latitude_atendente && localizacao.longitude_atendente) {
                        ...
                        if (!marcaEmergencia) {
                            marcaEmergencia = new google.maps.Marker({
                             ...
                             new google.maps.InfoWindow({ content: "Emergência!" }).open(mapa, marcaEmergencia);
                        } else {
                            marcaEmergencia.setPosition(coordenadas);
                        }
                    }
                })
                .catch(error => console.error("Erro ao obter localização da emergencia:", error));
        }
        document.getElementById('enviar-mensagem').addEventListener('click', function() {
            ...
            fetch(`usuario.php?id_emergencia=<?= $id_emergencia ?>&obter_mensagens=1`)
        .then(response => response.json())
        .then(mensagens => {
        ...
        function verificarStatus() {
            fetch(`emergencia.php?status_acao=1&id_emergencia=<?= $id_emergencia ?>`)
        .then(response => response.json())
        .then(status => {
